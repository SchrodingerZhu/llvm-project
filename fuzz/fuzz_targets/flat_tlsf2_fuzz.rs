...
fuzz_target!(|actions: Vec<Actions>| fuzz_flat_tlsf2(actions));

fn fuzz_flat_tlsf2(actions: Vec<Actions>) {
        ...
        match action {
            ...
            Actions::Realloc { index, new_size } => {
                if !allocations.is_empty() && new_size > 0 {
                    ...
                    if !new_ptr.is_null() {
                        // Check base chunk unit alignment correctness (32B on 64-bit)
                        assert_eq!(new_ptr as usize % 32, 0, "Reallocated pointer is not chunk-aligned (32B)!");
                        ...
                        alloc.ptr = new_ptr;
                        alloc.size = new_size;
                        alloc.alignment = 32; // realloc uses standard chunk alignment (32B)
                    }
                }
            }
        }
