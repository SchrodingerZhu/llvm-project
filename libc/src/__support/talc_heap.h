...
struct Node {
  ...
  LIBC_INLINE static Node **addr_of_next(Node *ptr) {
    return &(ptr->next);
  }
...
LIBC_INLINE constexpr size_t required_chunk_size(size_t size, size_t alignment) {
  size_t max_padding = (alignment > CHUNK_UNIT) ? (alignment - CHUNK_UNIT) : 0;
  return align_up(size + sizeof(Header) + sizeof(Tag) + max_padding, CHUNK_UNIT);
}
...
linear_extent_then_linearly_divided_exponential_binning(size_t size) {
  static_assert((LIN_DIVS & (LIN_DIVS - 1)) == 0, "LIN_DIVS must be power of 2");
...
template <size_t BUFF_SIZE> class TalcHeapBuffer : public TalcHeap {
public:
...
LIBC_INLINE void TalcHeap::register_gap(cpp::byte *base, cpp::byte *gap_end) {
  ...
  if (size < 32) {
    uint64_t val = size | (static_cast<uint64_t>(Tag::ALLOCATED_FLAG | Tag::HOLE_FLAG) << 56);
  ...
  Node::link_at(gap_base_to_node(base),
                Node{*bin_ptr, bin_ptr});
...
TalcHeap::full_search_bin(uint32_t bin, size_t actual_size_needed,
  ...
  while (current != nullptr) {
    ...
    size &= ~END_FLAG;
    size &= 0x00FFFFFFFFFFFFFFULL; // Mask out tag flags if any (should be 0 for free, but be safe)
...
LIBC_INLINE TALC_NOINLINE void *TalcHeap::allocate_impl(size_t alignment, size_t size) {
  if (size == 0)
    return nullptr;
  ...
  size_t actual_size_needed = align_up(size + sizeof(Header) + sizeof(Tag), CHUNK_UNIT);
  ...
  if (bin >= DefaultBinning::BIN_COUNT - 1) {
    ...
    if (b >= DefaultBinning::BIN_COUNT) {
      ...
      if (alignment <= CHUNK_UNIT) {
        ...
        while (true) {
          if (auto success = full_search_bin(b, actual_size_needed, alignment)) {
            base = success->base;
            chunk_end = success->end;
            break;
          }
