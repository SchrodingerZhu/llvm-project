...
template <bool EntropyProtection> class WyHash {
  ...
  static uint64_t wyhash(const void *key, size_t length, uint64_t seed,
    ...
    // Golang's implementation mixes a different secret here, but we keep it the
    // same as the original wyhash to make it easier to examine the correctness
    // via testing known values.
    return mix(secret[1] ^ length, mix(a ^ secret[1], b ^ seed));
  }
