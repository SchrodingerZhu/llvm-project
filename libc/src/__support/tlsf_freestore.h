...
TLSFFreeStoreImpl<CONFIG>::size_to_bit_index(size_t size) {
  ...
  // row index (determined by the highest set bit, log_size) and a linear
  ...
  //
  //   shifted_size >> (log_size - NUM_STEP_BITS) = NUM_STEPS + column_index
  //
  ...
  //
  //   index = EXP_BASE + NUM_STEPS * (log_size - EXP_BASE_LOG2 - 1)
  //                    + (shifted_size >> (log_size - NUM_STEP_BITS))
  //
  // Key Properties:
  // 1. Direct Leading Zero: Avoids pre-shifting the size variable before
  //    counting leading zeros, saving one shift instruction.
  // 2. No Base Subtractions: Avoids subtracting the power-of-two row base,
  //    reducing dynamic data hazards.
  // 3. Underflow Safety: When log_size == EXP_BASE_LOG2, the subtraction term
  //    (log_size - EXP_BASE_LOG2 - 1) underflows to two's complement -1 (which
  //    shifts to -NUM_STEPS), perfectly acting as subtraction in standard
  ...
  //   - Let size = 2100 bytes (shifted_size = 131)
  //   - Raw version:
  ...
  //   - Optimized version:
  //       log_size = 7
  //       exp_offset = (7 - 6 - 1) << 4 = 0
  //       step_index = 131 >> (7 - 4) = 16
  //       index = 64 + 0 + 16 = 80
  // =========================================================================
  ...
  size_t log_size = static_cast<size_t>(cpp::bit_width(shifted_size) - 1);
  size_t exp_offset = (log_size - EXP_BASE_LOG2 - 1) << CONFIG::NUM_STEP_BITS;
  size_t step_index = shifted_size >> (log_size - CONFIG::NUM_STEP_BITS);
