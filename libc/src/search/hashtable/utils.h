...
using RawTable =
    cpp::swisstable::RawTable<ENTRY, LLVM_LIBC_SEARCH_ENABLE_HASHTABLE_RESIZE,
...
using Bucket = cpp::swisstable::Bucket<ENTRY>;
...
struct Hash {
  ...
  uint64_t operator()(const ENTRY &entry) const {
    ...
    return cpp::wyhash::DefaultHash::hash(entry.key, length, seed);
  }
};
