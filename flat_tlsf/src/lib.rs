...
extern "C" {
    fn flat_tlsf_create(mem: *mut c_void, size: usize) -> *mut c_void;
    fn flat_tlsf_destroy(heap: *mut c_void);
    fn flat_tlsf_allocate(heap: *mut c_void, size: usize) -> *mut c_void;
    fn flat_tlsf_aligned_allocate(heap: *mut c_void, alignment: usize, size: usize) -> *mut c_void;
    fn flat_tlsf_free(heap: *mut c_void, ptr: *mut c_void);
    fn flat_tlsf_realloc(heap: *mut c_void, ptr: *mut c_void, size: usize) -> *mut c_void;
    fn flat_tlsf_calloc(heap: *mut c_void, num: usize, size: usize) -> *mut c_void;
    fn flat_tlsf_get_free_mem(heap: *mut c_void) -> usize;

    // FreeList FFI entry points
    fn freelist_create(mem: *mut c_void, size: usize) -> *mut c_void;
    fn freelist_destroy(heap: *mut c_void);
    fn freelist_allocate(heap: *mut c_void, size: usize) -> *mut c_void;
    fn freelist_aligned_allocate(heap: *mut c_void, alignment: usize, size: usize) -> *mut c_void;
    fn freelist_free(heap: *mut c_void, ptr: *mut c_void);
    fn freelist_realloc(heap: *mut c_void, ptr: *mut c_void, size: usize) -> *mut c_void;
    fn freelist_calloc(heap: *mut c_void, num: usize, size: usize) -> *mut c_void;
}
...
impl FlatTlsfHeap {
    ...
    pub fn new(capacity: usize) -> Self {
        ...
        let ffi_ptr = unsafe {
            flat_tlsf_create(storage.as_mut_ptr() as *mut _, capacity)
        };
    ...
    pub unsafe fn from_raw(mem: &mut [u8]) -> Self {
        ...
        let ffi_ptr = flat_tlsf_create(mem.as_mut_ptr() as *mut _, mem.len());
        assert!(!ffi_ptr.is_null(), "Failed to initialize FlatTlsfHeap inside C++ FFI");
        Self {
            ffi_ptr,
            _storage: None,
    ...
    pub unsafe fn allocate(&self, size: usize) -> *mut u8 {
        ...
        flat_tlsf_allocate(self.ffi_ptr, size) as *mut u8
    }
    ...
    pub unsafe fn aligned_allocate(&self, alignment: usize, size: usize) -> *mut u8 {
        ...
        flat_tlsf_aligned_allocate(self.ffi_ptr, alignment, aligned_size) as *mut u8
    }
    ...
    pub unsafe fn deallocate(&self, ptr: *mut u8) {
        ...
        flat_tlsf_free(self.ffi_ptr, ptr as *mut c_void);
    }
    ...
    pub unsafe fn reallocate(&self, ptr: *mut u8, new_size: usize) -> *mut u8 {
        flat_tlsf_realloc(self.ffi_ptr, ptr as *mut c_void, new_size) as *mut u8
    }
...
impl Drop for FlatTlsfHeap {
    fn drop(&mut self) {
        unsafe {
            flat_tlsf_destroy(self.ffi_ptr);
        }
    }
}
...
impl FreeListHeap {
    pub fn new(capacity: usize) -> Self {
        ...
        let mut storage = vec![0u8; capacity];
        let ffi_ptr = unsafe {
            freelist_create(storage.as_mut_ptr() as *mut _, capacity)
        };
    ...
    pub unsafe fn from_raw(mem: &mut [u8]) -> Self {
        assert!(mem.len() >= 32);
        let ffi_ptr = freelist_create(mem.as_mut_ptr() as *mut _, mem.len());
        assert!(!ffi_ptr.is_null());
        Self {
            ffi_ptr,
            _storage: None,
    ...
    pub unsafe fn allocate(&self, size: usize) -> *mut u8 {
        ...
        freelist_allocate(self.ffi_ptr, size) as *mut u8
    }
    ...
    pub unsafe fn aligned_allocate(&self, alignment: usize, size: usize) -> *mut u8 {
        ...
        let aligned_size = (size + alignment - 1) & !(alignment - 1);
        freelist_aligned_allocate(self.ffi_ptr, alignment, aligned_size) as *mut u8
    }
    ...
    pub unsafe fn deallocate(&self, ptr: *mut u8) {
        ...
        freelist_free(self.ffi_ptr, ptr as *mut c_void);
    }

    pub unsafe fn reallocate(&self, ptr: *mut u8, new_size: usize) -> *mut u8 {
        freelist_realloc(self.ffi_ptr, ptr as *mut c_void, new_size) as *mut u8
    }

    pub unsafe fn callocate(&self, num: usize, size: usize) -> *mut u8 {
        freelist_calloc(self.ffi_ptr, num, size) as *mut u8
    }
}
